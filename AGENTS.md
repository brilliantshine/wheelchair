...
`server.js` reads only `WHEELCHAIR_NO_BROWSER` and `WHEELCHAIR_BROWSER`, plus `TMUX`,
`TMUX_PANE` and `CLAUDECODE` to label who opened a graph — so exporting one puts you
straight back on the default root.
