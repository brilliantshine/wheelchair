...
| claude | minor | Decision 13's "per-repo so names cannot collide" is wrong for worktrees | `upheld` | `atlas-engine/development` and `atlas-engine/work/lane-a` are separate worktrees of one repo; `<repo-name>` is identical across them and §8 says a collision overwrites. Derivation of `<repo-name>` is also unstated. |
