...
| With an installed path close to 2,000 characters, the hook's fixed lines can leave no room under the cap and the wording list is left out | Verification rounds 5, 7 and 8 kept finding one more combination of lines that tips it over; Collin chose to describe it generally rather than exactly. The real install path is 43 characters | verification |
