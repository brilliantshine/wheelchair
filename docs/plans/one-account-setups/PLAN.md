...
| 4 | `verified-by` into the completion template; `**Lanes:**` slot into the plan template | `protocol/templates/COMPLETION.md`, `protocol/templates/PLAN.md` | luna | `01a04f1f-f4f9-7c61-be93-70cbd35ddd20` | format matches `plan-review.md`; lead fixed a populated frontmatter example that would have made every completion report assert an unperformed verification — brief defect, not lane defect | done |
| 5 | `protocol/sensitivity.md`'s six places, the four routers, three skill descriptions, `adopt.md:67`, two shell header comments | as listed | terra | `01a04f37-e03e-72e2-8fdb-2d0c545e6dfe` | both suites green and `./install.sh` idempotent, re-run by the lead (the lane could not run the real-home install under `workspace-write` and said so rather than claiming it); in-marker line read and its installed copy diffed | done |
| 6 | README: the opening sentence, the six stale spots, the dependency list, and a new section on what one account costs | `README.md` | sonnet | Agent tool | opening matches the Spec verbatim; lane found two further false claims the Spec's table missed (`:96-97`, `:104-111`) and left two design-property mentions alone with reasoning | done |
