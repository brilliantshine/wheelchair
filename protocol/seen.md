...
- The confirmed wording list — `## Confirmed` from `~/.wheelchair/wording.md` — newest
  first, with a trailing count of any entries left out to fit the cap. In degenerate cases
  where the fixed lines leave no room — only possible with an installed path close to 2,000
  characters long — the list is left out.
